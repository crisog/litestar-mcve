...
DURATION_VALUES: dict[DurationUnit, int] = {
    "second": 1,
    "minute": 60,
    "hour": 3600,
    "day": 86400,
...
class RateLimitMiddleware(AbstractMiddleware):
    ...
    def __init__(self, app: ASGIApp, config: RateLimitConfig) -> None:
        ...
        self.check_throttle_handler = cast(
            "Callable[[Request], Awaitable[bool]] | None", config.check_throttle_handler
        )
        self.config = config
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        Returns:
            None
        """
        print("rate limit middleware triggered")
        app = scope["app"]
    ...
    async def set_cached_history(
        self, key: str, cache_object: CacheObject, store: Store
    ) -> None:
        ...
        print("Setting cache on store:", store)
        await store.set(
            key, encode_json(cache_object), expires_in=DURATION_VALUES[self.unit]
        )
        print("Cache set")
    ...
    def create_response_headers(self, cache_object: CacheObject) -> dict[str, str]:
        ...
        remaining_requests = str(
            len(cache_object.history) - self.max_requests
            if len(cache_object.history) <= self.max_requests
            else 0
        )
        ...
        return {
            ...
            self.config.rate_limit_reset_header_key: str(
                int(time()) - cache_object.reset
            ),
        }
...
class RateLimitConfig:
    ...
    check_throttle_handler: (
        Callable[[Request[Any, Any, Any]], SyncOrAsyncUnion[bool]] | None
    ) = field(default=None)
